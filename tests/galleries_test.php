<?php
// php tests/galleries_test.php: temporary tables and test-owned files; providers mocked.
if (PHP_SAPI !== 'cli') { http_response_code(404); exit; }
if (!isset($argv[1])) {
    $cases = ['add_missing_caption','add_empty_caption','add_array_caption','add_none','add_empty_source','add_missing_image','add_empty_image','add_null_image','add_array_image','add_missing_url','add_empty_url','add_local','add_cloud','add_imagekit','add_external','add_disabled','add_pdf','add_long_caption','add_255_caption','add_rollback',
        'get_all','get_page','get_search','get_literal','get_order','get_empty','get_invalid',
        'update_empty_caption','update','update_same','update_missing','update_protected','update_long','update_bad_id',
        'sort_up','sort_down','sort_first','sort_last','sort_zero','sort_gap','sort_duplicate','sort_missing','sort_rollback',
        'delete','delete_missing','delete_cloud','delete_inactive','delete_shared_facility','delete_shared_gallery','delete_shared_teacher','delete_failure',
        'image_local','image_cloud','image_imagekit','image_external','image_delete','image_none_delete','image_inactive','image_shared_facility','image_shared_gallery','image_shared_teacher','image_cleanup_failure','image_upload_failure','image_rollback','image_race','image_missing','image_pdf',
        'auth','method','options'];
...
require $root . '/_Helper/GalleryImageStorage.php';
$case = $argv[1];
...
$oldPath=$directory . '/gallery-test-' . bin2hex(random_bytes(12)) . '.png';
file_put_contents($oldPath,$png);
$oldSource=in_array($case,['delete_cloud','delete_inactive'],true) ? 'Cloudinary' : 'Local Directory';
$metadata=$oldSource==='Cloudinary' ? ['public_id'=>'old-gallery','resource_type'=>'Image'] : ['file_name'=>basename($oldPath),'file_mime_type'=>'image/png'];
...
$p->exec("INSERT INTO galleries (id,caption,id_file_manager,sort_order) VALUES (1,'Books 100%_!',1,1),(2,'Science',NULL,2),(3,'Assembly',NULL,3)");
...
if (in_array($case,['delete_shared_facility','image_shared_facility'],true)) {
    $p->exec("INSERT INTO facilities (id,title,description,id_file_manager) VALUES (1,'Shared','Shared gallery image',1)");
}
if (in_array($case,['delete_shared_gallery','image_shared_gallery'],true)) {
    $p->exec('UPDATE galleries SET id_file_manager=1 WHERE id=2');
}

class GalleryTestStorage extends GalleryImageStorage
{
    public static array $events=[];
    protected function uploadCloudinary(string $file,string $publicId,string $extension): array
    {
        if (strpos($publicId,'web_sekolah/galleries/')!==0) { throw new LogicException('Wrong Cloudinary folder'); }
    ...
    protected function imagekitRequest(string $method,array $fields=[],?string $fileId=null): array
    {
        ...
        if ($fields['folder']!=='/web_sekolah/galleries') { throw new LogicException('Wrong Imagekit folder'); }
        return ['fileId'=>'new-gallery','fileType'=>'image','url'=>'https://example.com/new.png'];
    }
    public function store(string $source,object $input): array
    {
        ...
        if ($case==='image_race') { $p->exec('UPDATE galleries SET id_file_manager=NULL WHERE id=1'); }
        return $result;
    }
    public function deleteStoredFile(string $source,$metadata): string
    {
        ...
        if ($case==='image_cleanup_failure') {
            if ((int)$p->query('SELECT id_file_manager FROM galleries WHERE id=1')->fetchColumn()===1) { throw new LogicException('Cleanup before replacement'); }
...
$map=['add'=>'add_galleries.php','get'=>'get_galleries.php','update'=>'update_galleries.php','sort'=>'update_sort_order.php','delete'=>'delete_galleries.php','image'=>'update_image.php'];
$endpoint=$map[$kind] ?? 'add_galleries.php';
...
if ($kind==='add') {
    ...
    if (in_array($case,['add_missing_image','add_empty_image','add_null_image','add_array_image'],true)) {
        ...
        if ($case==='add_array_image') { $payload['base64']=[]; }
    }
    ...
    if ($case==='add_missing_caption') { unset($payload['caption']); $expected=400; }
    if ($case==='add_empty_caption') { $payload['caption']='  '; $expected=400; }
    if ($case==='add_array_caption') { $payload['caption']=[]; $expected=400; }
    if (in_array($case,['add_255_caption','add_local','add_cloud','add_imagekit','add_external','add_disabled','add_pdf','add_rollback'],true)) {
    ...
    if ($case==='add_long_caption') { $payload['caption']=str_repeat('a',256); $expected=400; }
    if ($case==='add_255_caption') { $payload['caption']=str_repeat('a',255); }
    if ($case==='add_rollback') { $p->exec("ALTER TABLE galleries ADD CONSTRAINT reject_new CHECK (caption <> 'New caption')"); $expected=500; }
}
if ($kind==='get') {
    if ($case==='get_page') { $_GET=['limit'=>'1','page'=>'2']; }
    if ($case==='get_search') { $_GET=['keyword'=>'Science']; }
    if ($case==='get_literal') { $_GET=['keyword'=>'%_!']; }
    if ($case==='get_order') { $_GET=['order_by'=>'caption','short_by'=>'DESC']; }
    if ($case==='get_empty') { $p->exec('DELETE FROM galleries'); }
    if ($case==='get_invalid') { $_GET=['order_by'=>'name']; $expected=400; }
}
if ($kind==='update') {
    if ($case==='update_empty_caption') { $payload['caption']='  '; $expected=400; }
    $payload['id']=1;
    if ($case==='update_same') { $payload=['id'=>1,'caption'=>'Books 100%_!']; }
    ...
    if ($case==='update_long') { $payload['caption']=str_repeat('a',256); $expected=400; }
}
if ($kind==='sort') {
    ...
    if ($case==='sort_zero') { $p->exec('UPDATE galleries SET sort_order=sort_order-1'); }
    if ($case==='sort_gap') { $p->exec('UPDATE galleries SET sort_order=sort_order*10'); }
    if ($case==='sort_duplicate') { $p->exec('UPDATE galleries SET sort_order=2 WHERE id=3'); $expected=409; }
    if ($case==='sort_missing') { $payload['id']=99; $expected=404; }
    if ($case==='sort_rollback') { $p->exec("ALTER TABLE galleries ADD CONSTRAINT reject_swap CHECK (caption <> 'Books 100%_!' OR sort_order=1)"); $expected=500; }
}
if ($kind==='delete') {
    $_GET=['id'=>'1'];
...
if ($kind==='image') {
    ...
    if ($case==='image_none_delete') { $p->exec('UPDATE galleries SET id_file_manager=NULL WHERE id=1'); }
    if ($case==='image_inactive') { $testConfig['imagekit_status']='Inactive'; $expected=403; }
    if ($case==='image_upload_failure') { $expected=502; }
    if ($case==='image_rollback') { $p->exec('ALTER TABLE galleries ADD CONSTRAINT reject_image CHECK (id_file_manager IS NULL OR id_file_manager=1)'); $expected=500; }
...
$before=$p->query('SELECT * FROM galleries ORDER BY id')->fetchAll();
$beforeFiles=glob($directory . '/*') ?: [];
$testBody=json_encode($payload);
$_SERVER['REQUEST_METHOD']=$method;
$_SERVER['CONTENT_TYPE']='application/json';
$_SERVER['SCRIPT_NAME']='/web-sekolah/_API/Galleries/' . $endpoint;
...
register_shutdown_function(function () use ($p,$case,$kind,$expected,$before,$beforeFiles,$directory,$oldPath,$oldSource,$payload) {
    ...
    $rows=$p->query('SELECT * FROM galleries ORDER BY id')->fetchAll();
    ...
    if ($expected>=400 || $expected===204) {
        ...
        $row=end($rows);
        $ok=$ok && count($rows)===4 && $row['caption']===$payload['caption'];
        ...
        if ($case==='get_order') { $ok=$ok && array_column($data,'id')===[2,1,3]; }
        if ($case==='get_empty') { $ok=$ok && $data===[] && $body['pagination']['total']===0; }
    } elseif ($kind==='update') {
        $ok=$ok && $rows[0]['caption']===$payload['caption'] && $rows[0]['id_file_manager']===1 && $rows[0]['sort_order']===1;
    } elseif ($kind==='sort') {
        ...
        $retained=in_array($case,['delete_inactive','delete_shared_teacher','delete_shared_facility','delete_shared_gallery'],true);
        ...
        if ($case==='delete_inactive' || $case==='delete_shared_teacher') { $ok=$ok && !in_array('cloud_delete:old-gallery',GalleryTestStorage::$events,true); }
        if ($case==='delete_cloud') { $ok=$ok && in_array('cloud_delete:old-gallery',GalleryTestStorage::$events,true); }
    } elseif ($kind==='image') {
        $retained=in_array($case,['image_none_delete','image_shared_teacher','image_shared_facility','image_shared_gallery','image_cleanup_failure'],true);
        $ok=$ok && $oldExists===$retained && is_file($oldPath)===$retained;
        $row=$rows[0];
        $ok=$ok && $row['caption']===$before[0]['caption'] && $row['sort_order']===$before[0]['sort_order'];
...
$code=file_get_contents($root . '/_API/Galleries/' . $endpoint);
$code=str_replace('__DIR__',var_export($root . '/_API/Galleries',true),$code);
...
$code=str_replace('new GalleryImageStorage($config)','new GalleryTestStorage($config)',$code);
eval(substr($code,5));
