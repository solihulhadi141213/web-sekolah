<?php
// php tests/testimonials_test.php: temporary tables and test-owned files; providers mocked.
if (PHP_SAPI !== 'cli') { http_response_code(404); exit; }
if (!isset($argv[1])) {
    $cases = ['add_empty_image','add_null_image','add_empty_provider_image','add_image_without_source','add_missing_name','add_missing_quote','add_unicode_quote','add_none','add_local','add_cloud','add_imagekit','add_external','add_disabled','add_pdf','add_long_parent_name','add_long_quote','add_max_quote','add_rollback',
        'get_all','get_page','get_search','get_literal','get_order','get_empty','get_invalid',
        'update','update_same','update_missing','update_protected','update_long','update_bad_id',
        'sort_up','sort_down','sort_first','sort_last','sort_zero','sort_gap','sort_duplicate','sort_missing','sort_rollback',
        'delete','delete_missing','delete_cloud','delete_inactive','delete_shared_testimonial','delete_shared_facility','delete_shared_teacher','delete_failure',
        'image_local','image_cloud','image_imagekit','image_external','image_delete','image_none_delete','image_inactive','image_shared_testimonial','image_shared_facility','image_shared_teacher','image_cleanup_failure','image_upload_failure','image_rollback','image_race','image_missing','image_pdf',
        'auth','method','options'];
...
require $root . '/_Helper/TestimonialImageStorage.php';
$case = $argv[1];
...
$oldPath=$directory . '/testimonial-test-' . bin2hex(random_bytes(12)) . '.png';
file_put_contents($oldPath,$png);
$oldSource=in_array($case,['delete_cloud','delete_inactive'],true) ? 'Cloudinary' : 'Local Directory';
$metadata=$oldSource==='Cloudinary' ? ['public_id'=>'old-testimonial','resource_type'=>'Image'] : ['file_name'=>basename($oldPath),'file_mime_type'=>'image/png'];
...
$p->exec("INSERT INTO testimonials (id,parent_name,quote,id_file_manager,sort_order) VALUES (1,'Library','Books 100%_!',1,1),(2,'Lab','Science',NULL,2),(3,'Hall','Assembly',NULL,3)");
...
if (strpos($case,'shared_testimonial') !== false) { $p->exec('UPDATE testimonials SET id_file_manager=1 WHERE id=2'); }
...
class TestimonialTestStorage extends TestimonialImageStorage
{
    public static array $events=[];
    protected function uploadCloudinary(string $file,string $publicId,string $extension): array
    {
        if (strpos($publicId,'web_sekolah/testimonials/')!==0) { throw new LogicException('Wrong Cloudinary folder'); }
    ...
    protected function imagekitRequest(string $method,array $fields=[],?string $fileId=null): array
    {
        ...
        if ($fields['folder']!=='/web_sekolah/testimonials') { throw new LogicException('Wrong Imagekit folder'); }
        return ['fileId'=>'new-testimonial','fileType'=>'image','url'=>'https://example.com/new.png'];
    }
    public function store(string $source,object $input): array
    {
        ...
        if ($case==='image_race') { $p->exec('UPDATE testimonials SET id_file_manager=NULL WHERE id=1'); }
        return $result;
    }
    public function deleteStoredFile(string $source,$metadata): string
    {
        ...
        if ($case==='image_cleanup_failure') {
            if ((int)$p->query('SELECT id_file_manager FROM testimonials WHERE id=1')->fetchColumn()===1) { throw new LogicException('Cleanup before replacement'); }
...
$map=['add'=>'add_testimonials.php','get'=>'get_testimonials.php','update'=>'update_testimonials.php','sort'=>'update_sort_order.php','delete'=>'delete_testimonials.php','image'=>'update_image.php'];
$endpoint=$map[$kind] ?? 'add_testimonials.php';
$method=['add'=>'POST','get'=>'GET','update'=>'PUT','sort'=>'PUT','delete'=>'DELETE','image'=>'PUT'][$kind] ?? 'POST';
$expected=$kind==='add' ? 201 : 200;
$payload=['parent_name'=>'New testimonial','quote'=>'New quote'];
$_GET=[];
if ($kind==='add') {
    if ($case==='add_empty_image') { $payload += ['file_source'=>'','base64'=>'','image_url'=>'']; }
    if ($case==='add_null_image') { $payload += ['file_source'=>null,'base64'=>null,'image_url'=>null]; }
    if ($case==='add_empty_provider_image') { $payload += ['file_source'=>'Local Directory','base64'=>'']; }
    if ($case==='add_image_without_source') { $payload['base64']=base64_encode($png); $expected=400; }
    if ($case==='add_missing_name') { unset($payload['parent_name']); $expected=400; }
    if ($case==='add_missing_quote') { unset($payload['quote']); $expected=400; }
    if ($case==='add_unicode_quote') { $payload['quote']=str_repeat("\xF0\x9F\x98\x80",16384); $expected=400; }
    ...
    if ($case==='add_long_parent_name') { $payload['parent_name']=str_repeat('a',101); $expected=400; }
    if ($case==='add_long_quote') { $payload['quote']=str_repeat('a',65536); $expected=400; }
    if ($case==='add_max_quote') { $payload['quote']=str_repeat('a',65535); }
    if ($case==='add_rollback') { $p->exec("ALTER TABLE testimonials ADD CONSTRAINT reject_new CHECK (parent_name <> 'New testimonial')"); $expected=500; }
}
if ($kind==='get') {
    if ($case==='get_page') { $_GET=['limit'=>'1','page'=>'2']; }
    if ($case==='get_search') { $_GET=['keyword'=>'Science']; }
    if ($case==='get_literal') { $_GET=['keyword'=>'%_!']; }
    if ($case==='get_order') { $_GET=['order_by'=>'parent_name','short_by'=>'DESC']; }
    if ($case==='get_empty') { $p->exec('DELETE FROM testimonials'); }
    if ($case==='get_invalid') { $_GET=['order_by'=>'name']; $expected=400; }
}
if ($kind==='update') {
    $payload['id']=1;
    if ($case==='update_same') { $payload=['id'=>1,'parent_name'=>'Library','quote'=>'Books 100%_!']; }
    ...
    if ($case==='update_long') { $payload['quote']=str_repeat('a',65536); $expected=400; }
}
if ($kind==='sort') {
    ...
    if ($case==='sort_zero') { $p->exec('UPDATE testimonials SET sort_order=sort_order-1'); }
    if ($case==='sort_gap') { $p->exec('UPDATE testimonials SET sort_order=sort_order*10'); }
    if ($case==='sort_duplicate') { $p->exec('UPDATE testimonials SET sort_order=2 WHERE id=3'); $expected=409; }
    if ($case==='sort_missing') { $payload['id']=99; $expected=404; }
    if ($case==='sort_rollback') { $p->exec("ALTER TABLE testimonials ADD CONSTRAINT reject_swap CHECK (parent_name <> 'Library' OR sort_order=1)"); $expected=500; }
}
if ($kind==='delete') {
    $_GET=['id'=>'1'];
...
if ($kind==='image') {
    ...
    if ($case==='image_none_delete') { $p->exec('UPDATE testimonials SET id_file_manager=NULL WHERE id=1'); }
    if ($case==='image_inactive') { $testConfig['imagekit_status']='Inactive'; $expected=403; }
    if ($case==='image_upload_failure') { $expected=502; }
    if ($case==='image_rollback') { $p->exec('ALTER TABLE testimonials ADD CONSTRAINT reject_image CHECK (id_file_manager IS NULL OR id_file_manager=1)'); $expected=500; }
...
$before=$p->query('SELECT * FROM testimonials ORDER BY id')->fetchAll();
$beforeFiles=glob($directory . '/*') ?: [];
$testBody=json_encode($payload);
$_SERVER['REQUEST_METHOD']=$method;
$_SERVER['CONTENT_TYPE']='application/json';
$_SERVER['SCRIPT_NAME']='/web-sekolah/_API/Testimonials/' . $endpoint;
...
register_shutdown_function(function () use ($p,$case,$kind,$expected,$before,$beforeFiles,$directory,$oldPath,$oldSource,$payload) {
    ...
    $rows=$p->query('SELECT * FROM testimonials ORDER BY id')->fetchAll();
    ...
    if ($expected>=400 || $expected===204) {
        ...
        $row=end($rows);
        $ok=$ok && count($rows)===4 && $row['parent_name']===$payload['parent_name'] && $row['quote']===$payload['quote'];
        if (isset($payload['file_source']) && !in_array($case,['add_empty_image','add_empty_provider_image'],true)) {
        ...
        $ok=$ok && $rows[0]['parent_name']===$payload['parent_name'] && $rows[0]['quote']===$payload['quote'] && $rows[0]['id_file_manager']===1 && $rows[0]['sort_order']===1;
    } elseif ($kind==='sort') {
        ...
        $retained=in_array($case,['delete_inactive','delete_shared_teacher','delete_shared_testimonial','delete_shared_facility'],true);
        ...
        if ($case==='delete_inactive' || $case==='delete_shared_teacher') { $ok=$ok && !in_array('cloud_delete:old-testimonial',TestimonialTestStorage::$events,true); }
        if ($case==='delete_cloud') { $ok=$ok && in_array('cloud_delete:old-testimonial',TestimonialTestStorage::$events,true); }
    } elseif ($kind==='image') {
        $retained=in_array($case,['image_none_delete','image_shared_teacher','image_shared_testimonial','image_shared_facility','image_cleanup_failure'],true);
        $ok=$ok && $oldExists===$retained && is_file($oldPath)===$retained;
        $row=$rows[0];
        $ok=$ok && $row['parent_name']===$before[0]['parent_name'] && $row['quote']===$before[0]['quote'] && $row['sort_order']===$before[0]['sort_order'];
...
$code=file_get_contents($root . '/_API/Testimonials/' . $endpoint);
$code=str_replace('__DIR__',var_export($root . '/_API/Testimonials',true),$code);
...
$code=str_replace('new TestimonialImageStorage($config)','new TestimonialTestStorage($config)',$code);
eval(substr($code,5));
