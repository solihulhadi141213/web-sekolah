<?php
// php tests/fasilitas_test.php: temporary tables and test-owned files; providers mocked.
if (PHP_SAPI !== 'cli') { http_response_code(404); exit; }
if (!isset($argv[1])) {
    $cases = ['add_none','add_local','add_cloud','add_imagekit','add_external','add_disabled','add_pdf','add_long_title','add_long_description','add_255_description','add_rollback',
        ...
        'delete','delete_missing','delete_cloud','delete_inactive','delete_shared_teacher','delete_failure',
        'image_local','image_cloud','image_imagekit','image_external','image_delete','image_none_delete','image_inactive','image_shared_teacher','image_cleanup_failure','image_upload_failure','image_rollback','image_race','image_missing','image_pdf',
        'auth','method','options'];
...
require $root . '/_Helper/FacilityImageStorage.php';
$case = $argv[1];
...
$oldPath=$directory . '/facility-test-' . bin2hex(random_bytes(12)) . '.png';
file_put_contents($oldPath,$png);
$oldSource=in_array($case,['delete_cloud','delete_inactive'],true) ? 'Cloudinary' : 'Local Directory';
$metadata=$oldSource==='Cloudinary' ? ['public_id'=>'old-facility','resource_type'=>'Image'] : ['file_name'=>basename($oldPath),'file_mime_type'=>'image/png'];
...
$p->exec("INSERT INTO facilities (id,title,description,id_file_manager,sort_order) VALUES (1,'Library','Books 100%_!',1,1),(2,'Lab','Science',NULL,2),(3,'Hall','Assembly',NULL,3)");
...
class FacilityTestStorage extends FacilityImageStorage
{
    public static array $events=[];
    protected function uploadCloudinary(string $file,string $publicId,string $extension): array
    {
        if (strpos($publicId,'web_sekolah/facilities/')!==0) { throw new LogicException('Wrong Cloudinary folder'); }
    ...
    protected function imagekitRequest(string $method,array $fields=[],?string $fileId=null): array
    {
        ...
        if ($fields['folder']!=='/web_sekolah/facilities') { throw new LogicException('Wrong Imagekit folder'); }
        return ['fileId'=>'new-facility','fileType'=>'image','url'=>'https://example.com/new.png'];
    }
    public function store(string $source,object $input): array
    {
        ...
        if ($case==='image_race') { $p->exec('UPDATE facilities SET id_file_manager=NULL WHERE id=1'); }
        return $result;
    }
    public function deleteStoredFile(string $source,$metadata): string
    {
        ...
        if ($case==='image_cleanup_failure') {
            if ((int)$p->query('SELECT id_file_manager FROM facilities WHERE id=1')->fetchColumn()===1) { throw new LogicException('Cleanup before replacement'); }
...
$map=['add'=>'add_fasilitas.php','get'=>'get_fasilitas.php','update'=>'update_fasilitas.php','sort'=>'update_sort_order.php','delete'=>'delete_fasilitas.php','image'=>'update_image.php'];
$endpoint=$map[$kind] ?? 'add_fasilitas.php';
$method=['add'=>'POST','get'=>'GET','update'=>'PUT','sort'=>'PUT','delete'=>'DELETE','image'=>'PUT'][$kind] ?? 'POST';
$expected=$kind==='add' ? 201 : 200;
$payload=['title'=>'New facility','description'=>'New description'];
$_GET=[];
if ($kind==='add') {
    ...
    if ($case==='add_long_description') { $payload['description']=str_repeat('a',256); $expected=400; }
    if ($case==='add_255_description') { $payload['description']=str_repeat('a',255); }
    if ($case==='add_rollback') { $p->exec("ALTER TABLE facilities ADD CONSTRAINT reject_new CHECK (title <> 'New facility')"); $expected=500; }
}
if ($kind==='get') {
    ...
    if ($case==='get_empty') { $p->exec('DELETE FROM facilities'); }
    if ($case==='get_invalid') { $_GET=['order_by'=>'name']; $expected=400; }
}
if ($kind==='update') {
    $payload['id']=1;
    if ($case==='update_same') { $payload=['id'=>1,'title'=>'Library','description'=>'Books 100%_!']; }
    ...
    if ($case==='update_long') { $payload['description']=str_repeat('a',256); $expected=400; }
}
if ($kind==='sort') {
    ...
    if ($case==='sort_zero') { $p->exec('UPDATE facilities SET sort_order=sort_order-1'); }
    if ($case==='sort_gap') { $p->exec('UPDATE facilities SET sort_order=sort_order*10'); }
    if ($case==='sort_duplicate') { $p->exec('UPDATE facilities SET sort_order=2 WHERE id=3'); $expected=409; }
    if ($case==='sort_missing') { $payload['id']=99; $expected=404; }
    if ($case==='sort_rollback') { $p->exec("ALTER TABLE facilities ADD CONSTRAINT reject_swap CHECK (title <> 'Library' OR sort_order=1)"); $expected=500; }
}
if ($kind==='delete') {
    $_GET=['id'=>'1'];
...
if ($kind==='image') {
    ...
    if ($case==='image_none_delete') { $p->exec('UPDATE facilities SET id_file_manager=NULL WHERE id=1'); }
    if ($case==='image_inactive') { $testConfig['imagekit_status']='Inactive'; $expected=403; }
    if ($case==='image_upload_failure') { $expected=502; }
    if ($case==='image_rollback') { $p->exec('ALTER TABLE facilities ADD CONSTRAINT reject_image CHECK (id_file_manager IS NULL OR id_file_manager=1)'); $expected=500; }
...
$before=$p->query('SELECT * FROM facilities ORDER BY id')->fetchAll();
$beforeFiles=glob($directory . '/*') ?: [];
$testBody=json_encode($payload);
$_SERVER['REQUEST_METHOD']=$method;
$_SERVER['CONTENT_TYPE']='application/json';
$_SERVER['SCRIPT_NAME']='/web-sekolah/_API/Fasilitas/' . $endpoint;
...
register_shutdown_function(function () use ($p,$case,$kind,$expected,$before,$beforeFiles,$directory,$oldPath,$oldSource,$payload) {
    ...
    $rows=$p->query('SELECT * FROM facilities ORDER BY id')->fetchAll();
    ...
    if ($expected>=400 || $expected===204) {
        ...
        $row=end($rows);
        $ok=$ok && count($rows)===4 && $row['title']===$payload['title'] && $row['description']===$payload['description'];
        ...
        $ok=$ok && $rows[0]['title']===$payload['title'] && $rows[0]['description']===$payload['description'] && $rows[0]['id_file_manager']===1 && $rows[0]['sort_order']===1;
    } elseif ($kind==='sort') {
        ...
        $retained=in_array($case,['delete_inactive','delete_shared_teacher'],true);
        ...
        if ($case==='delete_inactive' || $case==='delete_shared_teacher') { $ok=$ok && !in_array('cloud_delete:old-facility',FacilityTestStorage::$events,true); }
        if ($case==='delete_cloud') { $ok=$ok && in_array('cloud_delete:old-facility',FacilityTestStorage::$events,true); }
    } elseif ($kind==='image') {
        $retained=in_array($case,['image_none_delete','image_shared_teacher','image_cleanup_failure'],true);
        $ok=$ok && $oldExists===$retained && is_file($oldPath)===$retained;
        $row=$rows[0];
        $ok=$ok && $row['title']===$before[0]['title'] && $row['description']===$before[0]['description'] && $row['sort_order']===$before[0]['sort_order'];
...
$code=file_get_contents($root . '/_API/Fasilitas/' . $endpoint);
$code=str_replace('__DIR__',var_export($root . '/_API/Fasilitas',true),$code);
...
$code=str_replace('new FacilityImageStorage($config)','new FacilityTestStorage($config)',$code);
eval(substr($code,5));
