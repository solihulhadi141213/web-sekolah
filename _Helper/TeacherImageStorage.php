...
/** Penyimpanan gambar guru; metadata mengikuti Image-Metadata-Explanation.md. */
class TeacherImageStorage
{
    ...
    public function store(string $source, object $input): array
    {
        ...
        try {
            ...
            if ($source === 'Cloudinary') {
                $publicId = 'web_sekolah/teachers/' . pathinfo($name, PATHINFO_FILENAME);
            ...
            $result = $this->imagekitRequest('POST', [
                ...
                'folder' => '/web_sekolah/teachers', 'useUniqueFileName' => 'true',
            ]);
    ...
    protected function uploadCloudinary(string $file, string $publicId, string $extension): array
    {
        ...
        try {
            ...
            error_log('add_teachers: periksa unggahan Cloudinary ' . $publicId);
    ...
    protected function imagekitRequest(string $method, array $fields = [], ?string $fileId = null): array
    {
        ...
        if ($body === false || $status < 200 || $status >= 300) {
            ...
            error_log('add_teachers: operasi Imagekit gagal; method=' . $method . '; HTTP=' . $status);
