...
$hasReferences = static function (PDO $pdo, int $fileId, ?int $excludeTestimonial = null): bool {
    foreach (['teachers', 'articles', 'article_contents', 'facilities', 'galleries', 'hero_slides', 'testimonials'] as $table) {
        ...
        if ($table === 'testimonials' && $excludeTestimonial !== null) {
            $sql .= ' AND id <> :testimonial_id';
            $params['testimonial_id'] = $excludeTestimonial;
        }
...
$pdo = null;
$newStorage = new TestimonialImageStorage($config);
$oldStorage = new TestimonialImageStorage($config);
...
try {
    ...
    // Periksa testimonials dan metadata sebelum mengunggah; tidak menahan kunci saat upload.
    $selectTestimonial = $pdo->prepare('SELECT id_file_manager FROM testimonials WHERE id = :id');
    $selectTestimonial->execute(['id' => $id]);
    $testimonial = $selectTestimonial->fetch(PDO::FETCH_ASSOC);
    if (!$testimonial) {
        throw new RuntimeException('Data testimonials tidak ditemukan.', 404);
    }
    $oldFileId = $testimonial['id_file_manager'] === null ? null : (int) $testimonial['id_file_manager'];
    if ($oldFileId !== null) {
    ...
    $updateTestimonial = $pdo->prepare('UPDATE testimonials SET id_file_manager = :file_id WHERE id = :id');
    ...
    if ($currentFileId !== $oldFileId) {
        throw new RuntimeException('Gambar testimonials telah diubah oleh request lain. Muat ulang data sebelum mencoba kembali.', 409);
    }
    ...
    if ($deleting) {
        if ($oldFileId !== null) {
            ...
            if (!$oldFile) {
                throw new RuntimeException('Catatan file testimonials tidak ditemukan.', 409);
            }
        ...
        $updateTestimonial->execute(['file_id' => null, 'id' => $id]);
        ...
        $updateTestimonial->execute(['file_id' => $newFileId, 'id' => $id]);
    }
    $pdo->commit();
    $committed = true;
        ...
        try {
            ...
            if ($pdo->inTransaction()) {
                try { $pdo->rollBack(); } catch (Throwable $rollbackError) {
                    error_log('update_testimonials_image: rollback cleanup gagal; file_id=' . $oldFileId);
                }
            }
            ...
            error_log('update_testimonials_image: cleanup tertunda; testimonial_id=' . $id . '; file_id=' . $oldFileId);
        }
    }

    $respond(200, [
        'status' => 'success',
        'message' => $deleting ? 'Gambar testimonials berhasil dilepas.' : 'Gambar testimonials berhasil diperbarui.',
    ...
    if (!$committed && $safeToDeleteNew) {
        try { $newStorage->cleanup(); } catch (Throwable $cleanupError) {
            error_log('update_testimonials_image: pembersihan upload baru gagal; testimonial_id=' . ($id ?? 0));
        }
    } elseif (!$committed) {
        error_log('update_testimonials_image: status commit tidak pasti; periksa upload baru; testimonial_id=' . ($id ?? 0));
    }
    if ($oldStatus === 'deleted') {
        error_log('update_testimonials_image: periksa konsistensi file lama; file_id=' . $oldFileId);
    }
    ...
    error_log('update_testimonials_image: exception=' . get_class($error));
    $respond(500, ['status' => 'error', 'message' => 'Pembaruan gambar testimonials gagal. Periksa kembali status data dan file.']);
}
