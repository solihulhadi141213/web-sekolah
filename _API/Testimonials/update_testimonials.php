...
if (array_diff(array_keys(get_object_vars($input)), ['id', 'parent_name', 'quote'])) {
    $respond(400, ['status' => 'error', 'message' => 'Payload hanya boleh memuat id, parent_name, dan quote.']);
}
...
foreach (['parent_name' => 100, 'quote' => 65535] as $field => $maxLength) {
    ...
    if (($field === 'quote' ? strlen($value) : preg_match_all('/./us', $value)) > $maxLength) {
        $respond(400, ['status' => 'error', 'message' => $field . ' maksimal ' . $maxLength . ($field === 'quote' ? ' byte UTF-8.' : ' karakter.')]);
    }
    $fields[$field] = $value;
}
...
try {
    ...
    $stmt = $pdo->prepare('SELECT id FROM testimonials WHERE id = :id FOR UPDATE');
    ...
    // Hanya dua kolom informasi testimonials yang dapat diubah melalui endpoint ini.
    $stmt = $pdo->prepare('UPDATE testimonials SET parent_name = :parent_name, quote = :quote WHERE id = :id');
    $stmt->execute($fields + ['id' => $input->id]);
    $pdo->commit();

    $respond(200, [
        'status' => 'success',
        'message' => 'Informasi testimonials berhasil diperbarui.',
