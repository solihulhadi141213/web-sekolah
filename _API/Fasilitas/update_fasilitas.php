...
if (array_diff(array_keys(get_object_vars($input)), ['id', 'title', 'description'])) {
    $respond(400, ['status' => 'error', 'message' => 'Payload hanya boleh memuat id, title, dan description.']);
}
...
try {
    ...
    $stmt = $pdo->prepare('SELECT id FROM facilities WHERE id = :id FOR UPDATE');
    ...
    // Hanya dua kolom informasi fasilitas yang dapat diubah melalui endpoint ini.
    $stmt = $pdo->prepare('UPDATE facilities SET title = :title, description = :description WHERE id = :id');
    $stmt->execute($fields + ['id' => $input->id]);
    $pdo->commit();

    $respond(200, [
        'status' => 'success',
        'message' => 'Informasi fasilitas berhasil diperbarui.',
