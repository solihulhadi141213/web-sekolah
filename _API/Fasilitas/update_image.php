...
require_once __DIR__ . '/../../_Helper/FacilityImageStorage.php';
$userData = validateJWT($config);

// Pemanggil harus mengunci baris parent file_manager sebelum memeriksa referensi.
$hasReferences = static function (PDO $pdo, int $fileId, ?int $excludeFacility = null): bool {
    foreach (['teachers', 'articles', 'article_contents', 'facilities', 'galleries', 'hero_slides', 'testimonials'] as $table) {
        ...
        if ($table === 'facilities' && $excludeFacility !== null) {
            $sql .= ' AND id <> :facility_id';
            $params['facility_id'] = $excludeFacility;
        }
...
$pdo = null;
$newStorage = new FacilityImageStorage($config);
$oldStorage = new FacilityImageStorage($config);
...
try {
    ...
    // Periksa fasilitas dan metadata sebelum mengunggah; tidak menahan kunci saat upload.
    $selectFacility = $pdo->prepare('SELECT id_file_manager FROM facilities WHERE id = :id');
    $selectFacility->execute(['id' => $id]);
    $facility = $selectFacility->fetch(PDO::FETCH_ASSOC);
    if (!$facility) {
        throw new RuntimeException('Data fasilitas tidak ditemukan.', 404);
    }
    $oldFileId = $facility['id_file_manager'] === null ? null : (int) $facility['id_file_manager'];
    if ($oldFileId !== null) {
        ...
        if (!$stmt->fetch(PDO::FETCH_ASSOC)) {
            throw new RuntimeException('Catatan file fasilitas tidak ditemukan. Periksa relasi file_manager.', 409);
        }
    }
    $fileInsert = $pdo->prepare('INSERT INTO file_manager (file_source, file_metadata, creat_at) VALUES (:source, :metadata, UTC_TIMESTAMP())');
    $updateFacility = $pdo->prepare('UPDATE facilities SET id_file_manager = :file_id WHERE id = :id');
    $metadata = $deleting ? null : $newStorage->store($source, $input);
    unset($input);

    $pdo->beginTransaction();
    $safeToDeleteNew = false;
    $stmt = $pdo->prepare('SELECT id_file_manager FROM facilities WHERE id = :id FOR UPDATE');
    $stmt->execute(['id' => $id]);
    ...
    if ($currentFileId !== $oldFileId) {
        throw new RuntimeException('Gambar fasilitas telah diubah oleh request lain. Muat ulang data sebelum mencoba kembali.', 409);
    }
    ...
    if ($deleting) {
        if ($oldFileId !== null) {
            ...
            if (!$oldFile) {
                throw new RuntimeException('Catatan file fasilitas tidak ditemukan.', 409);
            }
        ...
        $updateFacility->execute(['file_id' => null, 'id' => $id]);
        ...
        $updateFacility->execute(['file_id' => $newFileId, 'id' => $id]);
    }
    $pdo->commit();
    $committed = true;
        ...
        try {
            ...
            if ($pdo->inTransaction()) {
                try { $pdo->rollBack(); } catch (Throwable $rollbackError) {
                    error_log('update_facilities_image: rollback cleanup gagal; file_id=' . $oldFileId);
                }
            }
            ...
            error_log('update_facilities_image: cleanup tertunda; facility_id=' . $id . '; file_id=' . $oldFileId);
        }
    }

    $respond(200, [
        'status' => 'success',
        'message' => $deleting ? 'Gambar fasilitas berhasil dilepas.' : 'Gambar fasilitas berhasil diperbarui.',
    ...
    if (!$committed && $safeToDeleteNew) {
        try { $newStorage->cleanup(); } catch (Throwable $cleanupError) {
            error_log('update_facilities_image: pembersihan upload baru gagal; facility_id=' . ($id ?? 0));
        }
    } elseif (!$committed) {
        error_log('update_facilities_image: status commit tidak pasti; periksa upload baru; facility_id=' . ($id ?? 0));
    }
    if ($oldStatus === 'deleted') {
        error_log('update_facilities_image: periksa konsistensi file lama; file_id=' . $oldFileId);
    }
    ...
    error_log('update_facilities_image: exception=' . get_class($error));
    $respond(500, ['status' => 'error', 'message' => 'Pembaruan gambar fasilitas gagal. Periksa kembali status data dan file.']);
}
