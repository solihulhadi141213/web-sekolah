...
try {
    ...
    if (!$facility) {
        $pdo->rollBack();
        $respond(404, ['status' => 'error', 'message' => 'Data fasilitas tidak ditemukan.']);
    }

    $fileId = $facility['id_file_manager'] === null ? null : (int) $facility['id_file_manager'];
    ...
    if ($pdo !== null && $pdo->inTransaction()) {
        try {
            ...
            error_log('delete_facilities: rollback gagal; facility_id=' . $id);
        }
    }
    // Transaksi SQL tidak dapat mengembalikan file yang sudah dihapus di storage.
    if ($fileStatus === 'deleted') {
        error_log('delete_facilities: rekonsiliasi diperlukan; facility_id=' . $id . '; file_id=' . $fileId);
    }
    ...
    error_log('delete_facilities: exception=' . get_class($error));
    $respond(500, ['status' => 'error', 'message' => 'Penghapusan data fasilitas gagal. Silakan periksa kembali status data dan file.']);
}
