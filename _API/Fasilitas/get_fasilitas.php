...
if (!in_array($orderBy, ['id', 'title', 'description', 'sort_order'], true)) {
    $respond(400, [
        'status' => 'error',
        'message' => 'Order_by harus salah satu dari: id, title, description, sort_order.'
    ]);
}
...
if ($keyword !== '') {
    ...
    $where = " WHERE (
        ...
        OR t.`description` LIKE :keyword_description ESCAPE '!'
    )";
    $bindings = [
        ':keyword_id' => $search,
        ':keyword_title' => $search,
        ':keyword_description' => $search
    ];
}
...
try {
    ...
    $countStmt = $pdo->prepare('SELECT COUNT(*) FROM `facilities` t' . $where);
    $countStmt->execute($bindings);
    $total = (int) $countStmt->fetchColumn();

    // ID menjadi penfasilitastan tambahan agar urutan nilai yang sama konsisten.
    ...
    $stmt = $pdo->prepare(
        'SELECT t.`id`, t.`title`, t.`description`, t.`id_file_manager`, t.`sort_order`, f.`file_source`, f.`file_metadata`
         FROM `facilities` t LEFT JOIN `file_manager` f ON f.`id_file_manager` = t.`id_file_manager`' . $where . '
    ...
    $scriptName = str_replace('\\', '/', $_SERVER['SCRIPT_NAME'] ?? '/_API/Fasilitas/get_fasilitas.php');
    $basePath = rtrim(str_replace('\\', '/', dirname($scriptName, 3)), '/.');

    foreach ($facilities as &$facility) {
        $facility['id'] = (int) $facility['id'];
        $facility['id_file_manager'] = $facility['id_file_manager'] === null
            ? null : (int) $facility['id_file_manager'];
        $facility['sort_order'] = $facility['sort_order'] === null
            ? null : (int) $facility['sort_order'];

        // Kembalikan metadata sebagai objek JSON, bukan string JSON berlapis.
        $metadata = $facility['file_metadata'] === null ? null : json_decode($facility['file_metadata']);
        $facility['file_metadata'] = is_object($metadata) ? $metadata : null;
        $facility['image_url'] = null;
        if (!is_object($metadata)) {
            continue;
        }

        if ($facility['file_source'] === 'Local Directory') {
            $fileName = $metadata->file_name ?? null;
            if (is_string($fileName) && $fileName !== '' && $fileName !== '.' && $fileName !== '..'
                && strpbrk($fileName, "/\\\0") === false) {
                $facility['image_url'] = $basePath . '/assets/img/local_directory/' . rawurlencode($fileName);
            }
        } else {
            $url = null;
            if ($facility['file_source'] === 'External Link') {
                $url = $metadata->file_url ?? null;
            } elseif (in_array($facility['file_source'], ['Cloudinary', 'Imagekit'], true)) {
    ...
    $respond(200, [
        'status' => 'success',
        'message' => $facilities ? 'Data fasilitas berhasil ditampilkan.' : 'Data fasilitas tidak ditemukan.',
