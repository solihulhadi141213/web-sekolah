...
$pdo = null;
$storage = new FacilityImageStorage($config);
$safeToDelete = true;
$statusCode = 500;
$response = ['status' => 'error', 'message' => 'Gagal menyimpan data fasilitas.'];
...
try {
    ...
    $fields = [];
    foreach (['title' => 100, 'description' => 255] as $field => $maxLength) {
    ...
    $facilityInsert = $pdo->prepare('INSERT INTO facilities (title, description, id_file_manager, sort_order)
        VALUES (:title, :description, :id_file_manager, :sort_order)');
    ...
    $sortOrder = (int) $pdo->query('SELECT COALESCE(MAX(sort_order), 0) + 1 FROM facilities')->fetchColumn();
    $facilityInsert->execute($fields + ['id_file_manager' => $fileId, 'sort_order' => $sortOrder]);
    $id = (int) $pdo->lastInsertId();
    $pdo->commit();

    $statusCode = 201;
    $response = [
        'status' => 'success', 'message' => 'Data fasilitas berhasil ditambahkan.',
    ...
    if ($safeToDelete) {
        try {
            ...
            error_log('add_facilities: pembersihan aset gagal; perlu rekonsiliasi.');
        }
    } else {
        error_log('add_facilities: hasil transaksi tidak pasti; periksa aset dan database.');
    }
    if (get_class($error) === RuntimeException::class && in_array($error->getCode(), [400, 403, 413, 415, 422, 500, 502], true)) {
        ...
        error_log('add_facilities: exception=' . get_class($error));
    }
}
