...
require_once __DIR__ . '/../../_Helper/TeacherImageStorage.php';
$userData = validateJWT($config);

// Pemanggil harus mengunci baris parent file_manager sebelum memeriksa referensi.
$hasReferences = static function (PDO $pdo, int $fileId, ?int $excludeTeacher = null): bool {
    foreach (['teachers', 'articles', 'article_contents', 'facilities', 'galleries', 'hero_slides', 'testimonials'] as $table) {
        ...
        if ($table === 'teachers' && $excludeTeacher !== null) {
            $sql .= ' AND id <> :teacher_id';
            $params['teacher_id'] = $excludeTeacher;
        }
...
$pdo = null;
$newStorage = new TeacherImageStorage($config);
$oldStorage = new TeacherImageStorage($config);
...
try {
    ...
    // Periksa guru dan metadata sebelum mengunggah; tidak menahan kunci saat upload.
    $selectTeacher = $pdo->prepare('SELECT id_file_manager FROM teachers WHERE id = :id');
    $selectTeacher->execute(['id' => $id]);
    $teacher = $selectTeacher->fetch(PDO::FETCH_ASSOC);
    if (!$teacher) {
        throw new RuntimeException('Data guru tidak ditemukan.', 404);
    }
    $oldFileId = $teacher['id_file_manager'] === null ? null : (int) $teacher['id_file_manager'];
    if ($oldFileId !== null) {
        ...
        if (!$stmt->fetch(PDO::FETCH_ASSOC)) {
            throw new RuntimeException('Catatan file guru tidak ditemukan. Periksa relasi file_manager.', 409);
        }
    }
    $fileInsert = $pdo->prepare('INSERT INTO file_manager (file_source, file_metadata, creat_at) VALUES (:source, :metadata, UTC_TIMESTAMP())');
    $updateTeacher = $pdo->prepare('UPDATE teachers SET id_file_manager = :file_id WHERE id = :id');
    $metadata = $deleting ? null : $newStorage->store($source, $input);
    unset($input);

    $pdo->beginTransaction();
    $safeToDeleteNew = false;
    $stmt = $pdo->prepare('SELECT id_file_manager FROM teachers WHERE id = :id FOR UPDATE');
    $stmt->execute(['id' => $id]);
    ...
    if ($currentFileId !== $oldFileId) {
        throw new RuntimeException('Gambar guru telah diubah oleh request lain. Muat ulang data sebelum mencoba kembali.', 409);
    }
    ...
    if ($deleting) {
        if ($oldFileId !== null) {
            ...
            if (!$oldFile) {
                throw new RuntimeException('Catatan file guru tidak ditemukan.', 409);
            }
        ...
        $updateTeacher->execute(['file_id' => null, 'id' => $id]);
        ...
        $updateTeacher->execute(['file_id' => $newFileId, 'id' => $id]);
    }
    $pdo->commit();
    $committed = true;
        ...
        try {
            ...
            if ($pdo->inTransaction()) {
                try { $pdo->rollBack(); } catch (Throwable $rollbackError) {
                    error_log('update_teachers_image: rollback cleanup gagal; file_id=' . $oldFileId);
                }
            }
            ...
            error_log('update_teachers_image: cleanup tertunda; teacher_id=' . $id . '; file_id=' . $oldFileId);
        }
    }

    $respond(200, [
        'status' => 'success',
        'message' => $deleting ? 'Gambar guru berhasil dilepas.' : 'Gambar guru berhasil diperbarui.',
    ...
    if (!$committed && $safeToDeleteNew) {
        try { $newStorage->cleanup(); } catch (Throwable $cleanupError) {
            error_log('update_teachers_image: pembersihan upload baru gagal; teacher_id=' . ($id ?? 0));
        }
    } elseif (!$committed) {
        error_log('update_teachers_image: status commit tidak pasti; periksa upload baru; teacher_id=' . ($id ?? 0));
    }
    if ($oldStatus === 'deleted') {
        error_log('update_teachers_image: periksa konsistensi file lama; file_id=' . $oldFileId);
    }
    ...
    error_log('update_teachers_image: exception=' . get_class($error));
    $respond(500, ['status' => 'error', 'message' => 'Pembaruan gambar guru gagal. Periksa kembali status data dan file.']);
}
