...
try {
    ...
    $rows = $pdo->query('SELECT id, sort_order FROM teachers ORDER BY sort_order ASC, id ASC FOR UPDATE')->fetchAll(PDO::FETCH_ASSOC);
    $position = null;
    ...
    if ($oldOrder === null || (int) $oldOrder < 1) {
        throw new RuntimeException('Urutan guru tidak valid. Sort_order harus lebih besar dari nol.', 409);
    }
    $oldOrder = (int) $oldOrder;
    if ($input->sort_order === 'UP' && ($oldOrder === 1 || $position === 0)) {
        throw new RuntimeException('Guru sudah berada di posisi paling atas dan tidak dapat dipindahkan ke atas.', 409);
    }
    if ($input->sort_order === 'DOWN' && $position === count($rows) - 1) {
        throw new RuntimeException('Guru sudah berada di posisi terakhir dan tidak dapat dipindahkan ke bawah.', 409);
    }
    ...
    if ($neighbor['sort_order'] === null || $newOrder < 1) {
        throw new RuntimeException('Urutan guru di posisi tujuan tidak valid.', 409);
    }
    $matching = 0;
    foreach ($rows as $row) {
    ...
    if ($oldOrder === $newOrder || $matching !== 2) {
        throw new RuntimeException('Urutan asal atau tujuan digunakan lebih dari satu guru. Perbaiki urutan terlebih dahulu.', 409);
    }

    // Schema teachers tidak memiliki indeks unik sort_order; kedua update berada dalam satu transaksi.
    $update = $pdo->prepare('UPDATE teachers SET sort_order = :sort_order WHERE id = :id');
    ...
    $respond(200, [
        'status' => 'success',
        'message' => 'Posisi guru berhasil diubah.',
    ...
    $respond(500, ['status' => 'error', 'message' => 'Terjadi kesalahan pada server database. Posisi guru gagal diubah.']);
}
