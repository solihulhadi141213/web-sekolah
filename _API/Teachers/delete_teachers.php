...
try {
    ...
    $fileId = $teacher['id_file_manager'] === null ? null : (int) $teacher['id_file_manager'];
    ...
    if ($fileId !== null) {
        ...
        if ($shared) {
            ...
            $storage = new TeacherImageStorage($config);
    ...
    if ($pdo !== null && $pdo->inTransaction()) {
        try {
            ...
            error_log('delete_teachers: rollback gagal; teacher_id=' . $id);
        }
    }
    // Transaksi SQL tidak dapat mengembalikan file yang sudah dihapus di storage.
    if ($fileStatus === 'deleted') {
        error_log('delete_teachers: rekonsiliasi diperlukan; teacher_id=' . $id . '; file_id=' . $fileId);
    }
    ...
    error_log('delete_teachers: exception=' . get_class($error));
    $respond(500, ['status' => 'error', 'message' => 'Penghapusan data guru gagal. Silakan periksa kembali status data dan file.']);
}
