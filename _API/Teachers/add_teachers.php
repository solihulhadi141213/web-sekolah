...
$safeToDelete = true;
$statusCode = 500;
$response = ['status' => 'error', 'message' => 'Gagal menyimpan data guru.'];
...
try {
    ...
    $teacherInsert = $pdo->prepare('INSERT INTO teachers (name, role, subject, id_file_manager, sort_order)
        VALUES (:name, :role, :subject, :id_file_manager, :sort_order)');
    ...
    $teacherInsert->execute($fields + ['id_file_manager' => $fileId, 'sort_order' => $sortOrder]);
    ...
    if ($safeToDelete) {
        try {
            ...
            error_log('add_teachers: pembersihan aset gagal; perlu rekonsiliasi.');
        }
    } else {
        error_log('add_teachers: hasil transaksi tidak pasti; periksa aset dan database.');
    }
    if (get_class($error) === RuntimeException::class && in_array($error->getCode(), [400, 403, 413, 415, 422, 500, 502], true)) {
        ...
        error_log('add_teachers: exception=' . get_class($error));
    }
}
