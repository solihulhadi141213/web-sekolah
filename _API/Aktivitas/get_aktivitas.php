...
if (!in_array($orderBy, ['id', 'category', 'title', 'description', 'sort_order'], true)) {
    $respond(400, [
        'status' => 'error',
        'message' => 'Order_by harus salah satu dari: id, category, title, description, sort_order.'
    ]);
}
...
if ($keyword !== '') {
    ...
    $where = " WHERE (
        ...
        OR t.`category` LIKE :keyword_category ESCAPE '!'
...
if (!is_string($category) || !in_array($category, ['', 'organisasi','ekstrakurikuler','prestasi'], true)) {
    $respond(400, ['status'=>'error','message'=>'Category harus organisasi, ekstrakurikuler, atau prestasi.']);
}
if ($category !== '') {
    $where .= ($where === '' ? ' WHERE ' : ' AND ') . 't.category = :category';
...
try {
    ...
    $countStmt = $pdo->prepare('SELECT COUNT(*) FROM `student_activities` t' . $where);
    $countStmt->execute($bindings);
    $total = (int) $countStmt->fetchColumn();

    // ID menjadi penaktivitastan tambahan agar urutan nilai yang sama konsisten.
    ...
    $stmt = $pdo->prepare(
        'SELECT t.`id`, t.`category`, t.`title`, t.`description`, t.`sort_order`
         FROM `student_activities` t' . $where . '
    ...
    foreach ($student_activities as &$aktivitas) {
        $aktivitas['id'] = (int) $aktivitas['id'];
        $aktivitas['sort_order'] = $aktivitas['sort_order'] === null ? null : (int) $aktivitas['sort_order'];
    }
    unset($aktivitas);

    $respond(200, [
        'status' => 'success',
        'message' => $student_activities ? 'Data aktivitas berhasil ditampilkan.' : 'Data aktivitas tidak ditemukan.',
