...
try {
    ...
    $rows = $pdo->query('SELECT id, category, sort_order FROM student_activities ORDER BY id FOR UPDATE')->fetchAll(PDO::FETCH_ASSOC);
    $category = null;
    foreach ($rows as $row) {
        if ((int) $row['id'] === $input->id) { $category = $row['category']; break; }
    }
    $rows = array_values(array_filter($rows, static function ($row) use ($category) { return $row['category'] === $category; }));
    usort($rows, static function ($a, $b) { return ($a['sort_order'] <=> $b['sort_order']) ?: ($a['id'] <=> $b['id']); });
    $position = null;
    ...
    if ($position === null) {
        throw new RuntimeException('Data aktivitas tidak ditemukan.', 404);
    }
    $oldOrder = $rows[$position]['sort_order'];
    if ($oldOrder === null || (int) $oldOrder < 0) {
        throw new RuntimeException('Urutan aktivitas tidak valid. Sort_order tidak boleh negatif.', 409);
    }
    ...
    if ($neighbor['sort_order'] === null || $newOrder < 0) {
        throw new RuntimeException('Urutan aktivitas di posisi tujuan tidak valid.', 409);
    }
    $matching = 0;
    foreach ($rows as $row) {
    ...
    if ($oldOrder === $newOrder || $matching !== 2) {
        throw new RuntimeException('Urutan asal atau tujuan digunakan lebih dari satu aktivitas. Perbaiki urutan terlebih dahulu.', 409);
    }

    // Schema student_activities tidak memiliki indeks unik sort_order; kedua update berada dalam satu transaksi.
    $update = $pdo->prepare('UPDATE student_activities SET sort_order = :sort_order WHERE id = :id');
    ...
    $respond(200, [
        'status' => 'success',
        'message' => 'Posisi aktivitas berhasil diubah.',
    ...
    $respond(500, ['status' => 'error', 'message' => 'Terjadi kesalahan pada server database. Posisi aktivitas gagal diubah.']);
}
