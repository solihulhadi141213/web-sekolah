...
if (array_diff(array_keys(get_object_vars($input)), ['id', 'title', 'subtitle', 'is_active'])) {
    $respond(400, ['status' => 'error', 'message' => 'Payload hanya boleh memuat id, title, subtitle, dan is_active.']);
}
...
$fields = [];
foreach (['title' => 100, 'subtitle' => 255] as $field => $maxLength) {
...
if (property_exists($input, 'is_active')) {
    if (!in_array($input->is_active, [0, 1], true)) {
        $respond(400, ['status' => 'error', 'message' => 'Is_active harus berupa integer 0 atau 1.']);
    }
...
try {
    ...
    $stmt = $pdo->prepare('SELECT id, is_active FROM hero_slides WHERE id = :id FOR UPDATE');
    ...
    // Jika status tidak dikirim, pertahankan status sebelumnya.
    $fields['is_active'] = $fields['is_active'] ?? ($existing['is_active'] === null ? null : (int) $existing['is_active']);
    $stmt = $pdo->prepare('UPDATE hero_slides SET title = :title, subtitle = :subtitle, is_active = :is_active WHERE id = :id');
    $stmt->execute($fields + ['id' => $input->id]);
    $pdo->commit();

    $respond(200, [
        'status' => 'success',
        'message' => 'Informasi hero berhasil diperbarui.',
