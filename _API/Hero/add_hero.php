...
$safeToDelete = true;
$statusCode = 500;
$response = ['status' => 'error', 'message' => 'Gagal menyimpan data hero.'];
...
try {
    ...
    if (property_exists($input, 'is_active') && !in_array($input->is_active, [0, 1], true)) {
    ...
    $heroInsert = $pdo->prepare('INSERT INTO hero_slides (title, subtitle, is_active, id_file_manager, sort_order)
    ...
    $statusCode = 201;
    $response = [
        'status' => 'success', 'message' => 'Data hero berhasil ditambahkan.',
    ...
    if ($safeToDelete) {
        try {
            ...
            error_log('add_hero_slides: pembersihan aset gagal; perlu rekonsiliasi.');
        }
    } else {
        error_log('add_hero_slides: hasil transaksi tidak pasti; periksa aset dan database.');
    }
    if (get_class($error) === RuntimeException::class && in_array($error->getCode(), [400, 403, 413, 415, 422, 500, 502], true)) {
        ...
        error_log('add_hero_slides: exception=' . get_class($error));
    }
}
