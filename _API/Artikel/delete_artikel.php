...
try {
    ...
    // Konten belum dikelola endpoint ini; cegah penghapusan cascade tak disengaja.
    $contents = $pdo->prepare('SELECT id FROM article_contents WHERE article_id = :id LIMIT 1 FOR UPDATE');
    $contents->execute(['id'=>$id]);
    if ($contents->fetchColumn() !== false) throw new RuntimeException('Artikel memiliki article_contents; penghapusan konten belum didukung.', 409);
    $fileId = $article['id_file_manager'] === null ? null : (int) $article['id_file_manager'];
    ...
    $pdo->prepare('DELETE FROM article_tag_map WHERE article_id = :id')->execute(['id'=>$id]);
    ...
    if ($pdo !== null && $pdo->inTransaction()) {
        try {
            ...
            error_log('delete_articles: rollback gagal; article_id=' . $id);
        }
    }
    // Transaksi SQL tidak dapat mengembalikan file yang sudah dihapus di storage.
    if ($fileStatus === 'deleted') {
        error_log('delete_articles: rekonsiliasi diperlukan; article_id=' . $id . '; file_id=' . $fileId);
    }
    ...
    error_log('delete_articles: exception=' . get_class($error));
    $respond(500, ['status' => 'error', 'message' => 'Penghapusan data artikel gagal. Silakan periksa kembali status data dan file.']);
}
