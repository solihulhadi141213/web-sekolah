...
$hasReferences = static function (PDO $pdo, int $fileId, ?int $excludeArticle = null): bool {
    foreach (['teachers', 'articles', 'article_contents', 'facilities', 'galleries', 'hero_slides', 'testimonials'] as $table) {
        ...
        if ($table === 'articles' && $excludeArticle !== null) {
            $sql .= ' AND id <> :article_id';
            $params['article_id'] = $excludeArticle;
        }
...
$pdo = null;
$newStorage = new ArticleImageStorage($config);
$oldStorage = new ArticleImageStorage($config);
...
try {
    ...
    // Periksa artikel dan metadata sebelum mengunggah; tidak menahan kunci saat upload.
    $selectArticle = $pdo->prepare('SELECT id_file_manager FROM articles WHERE id = :id');
    $selectArticle->execute(['id' => $id]);
    $article = $selectArticle->fetch(PDO::FETCH_ASSOC);
    if (!$article) {
        throw new RuntimeException('Data artikel tidak ditemukan.', 404);
    }
    $oldFileId = $article['id_file_manager'] === null ? null : (int) $article['id_file_manager'];
    if ($oldFileId !== null) {
        ...
        if (!$stmt->fetch(PDO::FETCH_ASSOC)) {
            throw new RuntimeException('Catatan file artikel tidak ditemukan. Periksa relasi file_manager.', 409);
        }
    }
    $fileInsert = $pdo->prepare('INSERT INTO file_manager (file_source, file_metadata, creat_at) VALUES (:source, :metadata, UTC_TIMESTAMP())');
    $updateArticle = $pdo->prepare('UPDATE articles SET id_file_manager = :file_id WHERE id = :id');
    $metadata = $deleting ? null : $newStorage->store($source, $input);
    unset($input);

    $pdo->beginTransaction();
    $safeToDeleteNew = false;
    $stmt = $pdo->prepare('SELECT id_file_manager FROM articles WHERE id = :id FOR UPDATE');
    $stmt->execute(['id' => $id]);
    ...
    if ($currentFileId !== $oldFileId) {
        throw new RuntimeException('Gambar artikel telah diubah oleh request lain. Muat ulang data sebelum mencoba kembali.', 409);
    }
    ...
    if ($deleting) {
        if ($oldFileId !== null) {
            ...
            if (!$oldFile) {
                throw new RuntimeException('Catatan file artikel tidak ditemukan.', 409);
            }
        ...
        $updateArticle->execute(['file_id' => null, 'id' => $id]);
        ...
        $updateArticle->execute(['file_id' => $newFileId, 'id' => $id]);
    }
    $pdo->commit();
    $committed = true;
        ...
        try {
            ...
            if ($pdo->inTransaction()) {
                try { $pdo->rollBack(); } catch (Throwable $rollbackError) {
                    error_log('update_articles_image: rollback cleanup gagal; file_id=' . $oldFileId);
                }
            }
            ...
            error_log('update_articles_image: cleanup tertunda; article_id=' . $id . '; file_id=' . $oldFileId);
        }
    }

    $respond(200, [
        'status' => 'success',
        'message' => $deleting ? 'Gambar artikel berhasil dilepas.' : 'Gambar artikel berhasil diperbarui.',
    ...
    if (!$committed && $safeToDeleteNew) {
        try { $newStorage->cleanup(); } catch (Throwable $cleanupError) {
            error_log('update_articles_image: pembersihan upload baru gagal; article_id=' . ($id ?? 0));
        }
    } elseif (!$committed) {
        error_log('update_articles_image: status commit tidak pasti; periksa upload baru; article_id=' . ($id ?? 0));
    }
    if ($oldStatus === 'deleted') {
        error_log('update_articles_image: periksa konsistensi file lama; file_id=' . $oldFileId);
    }
    ...
    error_log('update_articles_image: exception=' . get_class($error));
    $respond(500, ['status' => 'error', 'message' => 'Pembaruan gambar artikel gagal. Periksa kembali status data dan file.']);
}
