...
$hasReferences = static function (PDO $pdo, int $fileId, ?int $excludeGallery = null): bool {
    foreach (['teachers', 'articles', 'article_contents', 'facilities', 'galleries', 'hero_slides', 'testimonials'] as $table) {
        ...
        if ($table === 'galleries' && $excludeGallery !== null) {
            $sql .= ' AND id <> :gallery_id';
            $params['gallery_id'] = $excludeGallery;
        }
...
$pdo = null;
$newStorage = new GalleryImageStorage($config);
$oldStorage = new GalleryImageStorage($config);
...
try {
    ...
    // Periksa galleries dan metadata sebelum mengunggah; tidak menahan kunci saat upload.
    $selectGallery = $pdo->prepare('SELECT id_file_manager FROM galleries WHERE id = :id');
    $selectGallery->execute(['id' => $id]);
    $gallery = $selectGallery->fetch(PDO::FETCH_ASSOC);
    if (!$gallery) {
        throw new RuntimeException('Data galeri tidak ditemukan.', 404);
    }
    $oldFileId = $gallery['id_file_manager'] === null ? null : (int) $gallery['id_file_manager'];
    if ($oldFileId !== null) {
        ...
        if (!$stmt->fetch(PDO::FETCH_ASSOC)) {
            throw new RuntimeException('Catatan file galeri tidak ditemukan. Periksa relasi file_manager.', 409);
        }
    }
    $fileInsert = $pdo->prepare('INSERT INTO file_manager (file_source, file_metadata, creat_at) VALUES (:source, :metadata, UTC_TIMESTAMP())');
    $updateGallery = $pdo->prepare('UPDATE galleries SET id_file_manager = :file_id WHERE id = :id');
    $metadata = $deleting ? null : $newStorage->store($source, $input);
    unset($input);

    $pdo->beginTransaction();
    $safeToDeleteNew = false;
    $stmt = $pdo->prepare('SELECT id_file_manager FROM galleries WHERE id = :id FOR UPDATE');
    $stmt->execute(['id' => $id]);
    ...
    if ($currentFileId !== $oldFileId) {
        throw new RuntimeException('Gambar galeri telah diubah oleh request lain. Muat ulang data sebelum mencoba kembali.', 409);
    }
    ...
    if ($deleting) {
        if ($oldFileId !== null) {
            ...
            if (!$oldFile) {
                throw new RuntimeException('Catatan file galeri tidak ditemukan.', 409);
            }
        ...
        $updateGallery->execute(['file_id' => null, 'id' => $id]);
        ...
        $updateGallery->execute(['file_id' => $newFileId, 'id' => $id]);
    }
    $pdo->commit();
    $committed = true;
        ...
        try {
            ...
            if ($pdo->inTransaction()) {
                try { $pdo->rollBack(); } catch (Throwable $rollbackError) {
                    error_log('update_galleries_image: rollback cleanup gagal; file_id=' . $oldFileId);
                }
            }
            ...
            error_log('update_galleries_image: cleanup tertunda; gallery_id=' . $id . '; file_id=' . $oldFileId);
        }
    }

    $respond(200, [
        'status' => 'success',
        'message' => $deleting ? 'Gambar galeri berhasil dilepas.' : 'Gambar galeri berhasil diperbarui.',
    ...
    if (!$committed && $safeToDeleteNew) {
        try { $newStorage->cleanup(); } catch (Throwable $cleanupError) {
            error_log('update_galleries_image: pembersihan upload baru gagal; gallery_id=' . ($id ?? 0));
        }
    } elseif (!$committed) {
        error_log('update_galleries_image: status commit tidak pasti; periksa upload baru; gallery_id=' . ($id ?? 0));
    }
    if ($oldStatus === 'deleted') {
        error_log('update_galleries_image: periksa konsistensi file lama; file_id=' . $oldFileId);
    }
    ...
    error_log('update_galleries_image: exception=' . get_class($error));
    $respond(500, ['status' => 'error', 'message' => 'Pembaruan gambar galleries gagal. Periksa kembali status data dan file.']);
}
