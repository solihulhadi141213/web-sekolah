...
try {
    ...
    $rows = $pdo->query('SELECT id, sort_order FROM galleries ORDER BY sort_order ASC, id ASC FOR UPDATE')->fetchAll(PDO::FETCH_ASSOC);
    $position = null;
    ...
    if ($oldOrder === null || (int) $oldOrder < 0) {
        throw new RuntimeException('Urutan galeri tidak valid. Sort_order tidak boleh negatif.', 409);
    }
    $oldOrder = (int) $oldOrder;
    if ($input->sort_order === 'UP' && $position === 0) {
        throw new RuntimeException('Galeri sudah berada di posisi paling atas dan tidak dapat dipindahkan ke atas.', 409);
    }
    if ($input->sort_order === 'DOWN' && $position === count($rows) - 1) {
        throw new RuntimeException('Galeri sudah berada di posisi terakhir dan tidak dapat dipindahkan ke bawah.', 409);
    }
    ...
    if ($neighbor['sort_order'] === null || $newOrder < 0) {
        throw new RuntimeException('Urutan galeri di posisi tujuan tidak valid.', 409);
    }
    $matching = 0;
    foreach ($rows as $row) {
    ...
    if ($oldOrder === $newOrder || $matching !== 2) {
        throw new RuntimeException('Urutan asal atau tujuan digunakan lebih dari satu galeri. Perbaiki urutan terlebih dahulu.', 409);
    }

    // Schema galleries tidak memiliki indeks unik sort_order; kedua update berada dalam satu transaksi.
    $update = $pdo->prepare('UPDATE galleries SET sort_order = :sort_order WHERE id = :id');
    ...
    $respond(200, [
        'status' => 'success',
        'message' => 'Posisi galeri berhasil diubah.',
    ...
    $respond(500, ['status' => 'error', 'message' => 'Terjadi kesalahan pada server database. Posisi galeri gagal diubah.']);
}
