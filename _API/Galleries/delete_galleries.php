...
try {
    ...
    $fileId = $gallery['id_file_manager'] === null ? null : (int) $gallery['id_file_manager'];
    ...
    if ($pdo !== null && $pdo->inTransaction()) {
        try {
            ...
            error_log('delete_galleries: rollback gagal; gallery_id=' . $id);
        }
    }
    // Transaksi SQL tidak dapat mengembalikan file yang sudah dihapus di storage.
    if ($fileStatus === 'deleted') {
        error_log('delete_galleries: rekonsiliasi diperlukan; gallery_id=' . $id . '; file_id=' . $fileId);
    }
    ...
    error_log('delete_galleries: exception=' . get_class($error));
    $respond(500, ['status' => 'error', 'message' => 'Penghapusan data galeri gagal. Silakan periksa kembali status data dan file.']);
}
